...
export default function Industries() {
  return (
    ...
    >
            ...
            {segments.map((segment, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className="group bg-gradient-to-br from-white via-slate-50 to-slate-100 p-10 rounded-[2rem] border border-[var(--line-on-light)] shadow-xl relative overflow-hidden transition-all hover:shadow-2xl hover:-translate-y-1 hover:to-indigo-50/30"
                >
                  
                  <div className="w-16 h-16 rounded-2xl bg-[var(--surface-light-alt)] flex items-center justify-center mb-8 group-hover:scale-110 group-hover:bg-[var(--accent)]/10 transition-all duration-300">
                    <Icon size={32} className="text-[var(--text-on-light)] group-hover:text-[var(--brass)] transition-colors" />
                  </div>
                  
                  <h3 className="text-2xl font-bold text-[var(--text-on-light)] mb-4">{segment.title}</h3>
                  <p className="text-[var(--text-on-light)] leading-relaxed font-medium mb-8">
                    {segment.desc}
                  </p>
                  ...
                  <div className="space-y-3 mb-10">
                    {segment.features.map(feat => (
                      <div key={feat} className="flex items-center gap-3">
                        ...
                        <span className="text-[var(--text-on-light)] font-medium">{feat}</span>
                      </div>
                    ))}
                  </div>
      ...
      <FAQSection faqs={faqData.industries} title="Implementation FAQs" subtitle="Common questions about onboarding and deployment." />
      <CTASection />
    </PageWrapper>
  );
}
