...
export function AccordionItem({ question, answer, dark = false }) {
  ...
  return (
    <div className={cn(
      ...
      dark ? 'border-[#334155]/60' : 'border-slate-200/60'
    )}>
      <button
        onClick={() => setOpen((o) => !o)}
        aria-expanded={open}
        className={cn(
          ...
          dark
            ? 'text-white hover:text-[#C58A38]'
            : 'text-slate-900 hover:text-[#C58A38]'
        )}
      >
        <span>{question}</span>
        <motion.span
          ...
          className="shrink-0 text-[#C58A38]"
        >
          <ChevronDown size={18} />
        </motion.span>
      </button>
...
export default function Accordion({ items = [], dark = false, className = '' }) {
  return (
    <div className={cn(
      ...
      dark ? 'bg-[#1E293B]/80 backdrop-blur-xl px-8 shadow-xl shadow-slate-900/20' : 'bg-white/60 backdrop-blur-xl border border-slate-200/60 px-8 shadow-lg shadow-slate-200/40',
      className
    )}>
      {items.map((item, i) => (
